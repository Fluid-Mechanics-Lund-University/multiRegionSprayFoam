...
#include "dynamicMultiLevelRefineFvMesh.H"
...
namespace Foam
{
    defineTypeNameAndDebug(dynamicMultiLevelRefineFvMesh, 0);
    addToRunTimeSelectionTable(dynamicFvMesh, dynamicMultiLevelRefineFvMesh, IOobject);
}

// * * * * * * * * * * * * Protected Member Functions  * * * * * * * * * * * //

Foam::label Foam::dynamicMultiLevelRefineFvMesh::count
(
...
void Foam::dynamicMultiLevelRefineFvMesh::calculateProtectedCells
(
...
void Foam::dynamicMultiLevelRefineFvMesh::readDict()
{
...
Foam::dynamicMultiLevelRefineFvMesh::refine
(
...
Foam::autoPtr<Foam::mapPolyMesh>
Foam::dynamicMultiLevelRefineFvMesh::unrefine
(
...
Foam::scalarField
Foam::dynamicMultiLevelRefineFvMesh::maxPointField(const scalarField& pFld) const
{
...
Foam::scalarField
Foam::dynamicMultiLevelRefineFvMesh::maxCellField(const volScalarField& vFld) const
{
...
Foam::scalarField
Foam::dynamicMultiLevelRefineFvMesh::cellToPoint(const scalarField& vFld) const
{
...
Foam::scalarField Foam::dynamicMultiLevelRefineFvMesh::error
(
...
void Foam::dynamicMultiLevelRefineFvMesh::selectRefineCandidates
(
...
Foam::labelList Foam::dynamicMultiLevelRefineFvMesh::selectRefineCells
(
    const label maxCells,
    const volScalarField& maxRefinement,  //Shenghui 20200601 const label maxRefinement -> const volScalarField& maxRefinement
...
) const
{
    ...
    if (nCandidates < nTotToRefine)
    {
        forAll(candidateCell, celli)
        {
            if
            (
                cellLevel[celli] < maxRefinement[celli] // maxRefinement -> maxRefinement[celli] Shenghui 20200601
...
/*         for (label level = 0; level < maxRefinement; level++)
		...
		forAll(candidateCell, celli)
		{
			for (label level = 0; level < maxRefinement[celli]; level++)
			{
                if
                (
                    cellLevel[celli] == level
...
Foam::labelList Foam::dynamicMultiLevelRefineFvMesh::selectUnrefinePoints
(
...
void Foam::dynamicMultiLevelRefineFvMesh::extendMarkedCells
(
...
void Foam::dynamicMultiLevelRefineFvMesh::checkEightAnchorPoints
(
...
Foam::dynamicMultiLevelRefineFvMesh::dynamicMultiLevelRefineFvMesh(const IOobject& io)
:
    dynamicFvMesh(io),
    meshCutter_(*this),
    dumpLevel_(false),
...
Foam::dynamicMultiLevelRefineFvMesh::~dynamicMultiLevelRefineFvMesh()
{}


// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //

bool Foam::dynamicMultiLevelRefineFvMesh::update()
{
    ...
    if (time().timeIndex() > 0 && time().timeIndex() % refineInterval == 0)
    {
	   ...
	   //Shenghui 20200601
	   const volScalarField& maxRefinement = lookupObject<volScalarField>("maxRefinement");
...
bool Foam::dynamicMultiLevelRefineFvMesh::writeObject
(
